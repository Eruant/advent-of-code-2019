  ...
  wires.map(wire => {
    ...
    wire.map(instruction => {
      ...
      while (steps) {
        ...
        const key = `${y}:${x}`
        wirePositions.push(key)

        steps--
      }
    })

    positions.push(wirePositions)
  })

  return positions
}

const getCrossedWires = positions => {
  const [wire1, wire2] = positions

  return wire1.filter(position => wire2.includes(position))
}

const convertPositionsToDistance = positions =>
  positions
    .map(key =>
      key.split(':')
        .map(str => Math.abs(parseInt(str, 10)))
        .reduce((sum, current) => sum + current)
    )

const getClosest = distances => {
...
const positions = getPositions(wires)
const crossedWires = getCrossedWires(positions)
const distances = convertPositionsToDistance(crossedWires)
const closest = getClosest(distances)

// now take crossedWires and calculate steps taken for each wire to get there...
const getStepsToPoint = (wires, crossedWires) => {
  const stepsToCrossedWires = {}
  ...
  wires.map(wire => {
    ...
    wire.map(instruction => {
      ...
      while (steps) {
        ...
        if (crossedWires.includes(key)) {
          stepsToCrossedWires[key] = stepsToCrossedWires[key]
            ? stepsToCrossedWires[key] + totalSteps
            : totalSteps
        }
  ...
  return stepsToCrossedWires
}

const getLowestSteps = results =>
...
const stepsForCrosses = getStepsToPoint(wires, crossedWires)
const lowestSteps = getLowestSteps(stepsForCrosses)

console.log({part1: closest, part2: lowestSteps})
